...
describe ResponseSet do
  ...
  describe '#access_code' do
    let!(:rs1) { create(:response_set).tap { |rs| rs.update_attribute(:access_code, 'one') } }
    let!(:rs2) { create(:response_set).tap { |rs| rs.update_attribute(:access_code, 'two') } }
  ...
  it 'saves its responses' do
    new_set = ResponseSet.new(:survey => create(:survey))
...
describe ResponseSet, "with dependencies" do
  before(:each) do
    @section = create(:survey_section)
    # Questions
    @do_you_like_pie = create(:question, :text => "Do you like pie?", :survey_section => @section)
    @what_flavor = create(:question, :text => "What flavor?", :survey_section => @section)
    @what_bakery = create(:question, :text => "What bakery?", :survey_section => @section)
    # Answers
    @do_you_like_pie.answers << create(:answer, :text => "yes", :question_id => @do_you_like_pie.id)
    @do_you_like_pie.answers << create(:answer, :text => "no", :question_id => @do_you_like_pie.id)
    @what_flavor.answers << create(:answer, :response_class => :string, :question_id => @what_flavor.id)
    @what_bakery.answers << create(:answer, :response_class => :string, :question_id => @what_bakery.id)
    # Dependency
    @what_flavor_dep = create(:dependency, :rule => "A", :question_id => @what_flavor.id)
    create(:dependency_condition, :rule_key => "A", :question_id => @do_you_like_pie.id, :operator => "==", :answer_id => @do_you_like_pie.answers.first.id, :dependency_id => @what_flavor_dep.id)
    @what_bakery_dep = create(:dependency, :rule => "B", :question_id => @what_bakery.id)
    create(:dependency_condition, :rule_key => "B", :question_id => @do_you_like_pie.id, :operator => "==", :answer_id => @do_you_like_pie.answers.first.id, :dependency_id => @what_bakery_dep.id)
    # Responses
    @response_set = create(:response_set)
    @response_set.responses << create(:response, :question_id => @do_you_like_pie.id, :answer_id => @do_you_like_pie.answers.first.id, :response_set_id => @response_set.id)
    @response_set.responses << create(:response, :string_value => "pecan pie", :question_id => @what_flavor.id, :answer_id => @what_flavor.answers.first.id, :response_set_id => @response_set.id)
  end
  ...
  it "should list group as dependency" do
    # Question Group
    crust_group = create(:question_group, :text => "Favorite Crusts")

    # Question
    what_crust = create(:question, :text => "What is your favorite curst type?", :survey_section => @section)
    crust_group.questions << what_crust

    # Answers
    what_crust.answers << create(:answer, :response_class => :string, :question_id => what_crust.id)

    # Dependency
    crust_group_dep = create(:dependency, :rule => "C", :question_group_id => crust_group.id, :question => nil)
    create(:dependency_condition, :rule_key => "C", :question_id => @do_you_like_pie.id, :operator => "==", :answer_id => @do_you_like_pie.answers.first.id, :dependency_id => crust_group_dep.id)
...
describe ResponseSet, "dependency_conditions" do
  before do
    @section = create(:survey_section)
    # Questions
    @like_pie = create(:question, :text => "Do you like pie?", :survey_section => @section)
    @like_jam = create(:question, :text => "Do you like jam?", :survey_section => @section)
    @what_is_wrong_with_you = create(:question, :text => "What's wrong with you?", :survey_section => @section)
    # Answers
    @like_pie.answers << create(:answer, :text => "yes", :question_id => @like_pie.id)
    @like_pie.answers << create(:answer, :text => "no", :question_id => @like_pie.id)
    @like_jam.answers << create(:answer, :text => "yes", :question_id => @like_jam.id)
    @like_jam.answers << create(:answer, :text => "no", :question_id => @like_jam.id)
    # Dependency
    @what_is_wrong_with_you = create(:dependency, :rule => "A or B", :question_id => @what_is_wrong_with_you.id)
    @dep_a = create(:dependency_condition, :rule_key => "A", :question_id => @like_pie.id, :operator => "==", :answer_id => @like_pie.answers.first.id, :dependency_id => @what_is_wrong_with_you.id)
    @dep_b = create(:dependency_condition, :rule_key => "B", :question_id => @like_jam.id, :operator => "==", :answer_id => @like_jam.answers.first.id, :dependency_id => @what_is_wrong_with_you.id)
    # Responses
    @response_set = create(:response_set)
    @response_set.responses << create(:response, :question_id => @like_pie.id, :answer_id => @like_pie.answers.last.id, :response_set_id => @response_set.id)
  end
  ...
  it "should list all dependencies for passed question_id" do
    # Questions
    like_ice_cream = create(:question, :text => "Do you like ice_cream?", :survey_section => @section)
    what_flavor = create(:question, :text => "What flavor?", :survey_section => @section)
    # Answers
    like_ice_cream.answers << create(:answer, :text => "yes", :question_id => like_ice_cream.id)
    like_ice_cream.answers << create(:answer, :text => "no", :question_id => like_ice_cream.id)
    what_flavor.answers << create(:answer, :response_class => :string, :question_id => what_flavor.id)
    # Dependency
    flavor_dependency = create(:dependency, :rule => "C", :question_id => what_flavor.id)
    flavor_dependency_condition = create(:dependency_condition, :rule_key => "A", :question_id => like_ice_cream.id, :operator => "==",
...
describe ResponseSet, "as a quiz" do
  before(:each) do
    ...
    @response_set = create(:response_set, :survey => @survey)
  end
  def generate_responses(count, quiz = nil, correct = nil)
    count.times do |i|
      q = create(:question, :survey_section => @section)
      a = create(:answer, :question => q, :response_class => "answer")
      x = create(:answer, :question => q, :response_class => "answer")
      q.correct_answer = (quiz == "quiz" ? a : nil)
      @response_set.responses << create(:response, :question => q, :answer => (correct == "correct" ? a : x))
    end
  end
...
describe ResponseSet, "with mandatory questions" do
  ...
  def generate_responses(count, mandatory = nil, responded = nil)
    count.times do |i|
      q = create(:question, :survey_section => @section, :is_mandatory => (mandatory == "mandatory"))
      a = create(:answer, :question => q, :response_class => "answer")
      if responded == "responded"
        @response_set.responses << create(:response, :question => q, :answer => a)
      end
    end
  end
  ...
  it "should ignore labels and images" do
    generate_responses(3, "mandatory", "responded")
    create(:question, :survey_section => @section, :display_type => "label", :is_mandatory => true)
    create(:question, :survey_section => @section, :display_type => "image", :is_mandatory => true)
    expect(@response_set.mandatory_questions_complete?).to be_truthy
...
describe ResponseSet, "with mandatory, dependent questions" do
  ...
  def generate_responses(count, mandatory = nil, dependent = nil, triggered = nil)
    dq = create(:question, :survey_section => @section, :is_mandatory => (mandatory == "mandatory"))
    da = create(:answer, :question => dq, :response_class => "answer")
    dx = create(:answer, :question => dq, :response_class => "answer")
    count.times do |i|
      q = create(:question, :survey_section => @section, :is_mandatory => (mandatory == "mandatory"))
      a = create(:answer, :question => q, :response_class => "answer")
      if dependent == "dependent"
        d = create(:dependency, :question => q)
        dc = create(:dependency_condition, :dependency => d, :question_id => dq.id, :answer_id => da.id)
      end
      @response_set.responses << create(:response, :response_set => @response_set, :question => dq, :answer => (triggered == "triggered" ? da : dx))
      @response_set.responses << create(:response, :response_set => @response_set, :question => q, :answer => a)
    end
  end
  it "should report progress without mandatory questions" do
...
describe ResponseSet, "#as_json" do
  let(:rs) {
    create(:response_set, :responses => [
          create(:response, :question => create(:question), :answer => create(:answer), :string_value => '2')])
  }
